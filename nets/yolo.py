...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, backbone='cspdarknet', pretrained=False, input_shape=[640, 640]):
        ...
        """动态上采样替换nearest上采样"""
        # self.upsample1 = Dy_Sample(base_channels * 8, scale=2, style='lp', groups=4, dyscope=False)  # 512
        # self.upsample2 = Dy_Sample(base_channels * 4, scale=2, style='lp', groups=4, dyscope=False)  # 256

        self.conv_for_feat3         = Conv(base_channels * 16, base_channels * 8, 1, 1)
        self.conv3_for_upsample1    = DSC2f(base_channels * 16, base_channels * 8, base_depth, shortcut=False)

        self.conv_for_feat2         = Conv(base_channels * 8, base_channels * 4, 1, 1)
        self.conv3_for_upsample2    = DSC2f(base_channels * 8, base_channels * 4, base_depth, shortcut=False)

        self.down_sample1           = Conv(base_channels * 4, base_channels * 4, 3, 2)
        self.conv3_for_downsample1  = DSC2f(base_channels * 8, base_channels * 8, base_depth, shortcut=False)

        self.down_sample2           = Conv(base_channels * 8, base_channels * 8, 3, 2)
        self.conv3_for_downsample2  = DSC2f(base_channels * 16, base_channels * 16, base_depth, shortcut=False)
        ...
        """针对P2, P3, P4, P5层，添加MultiSEAM注意力模块"""
        # self.multi_seam_p3 = MultiSEAM(c1=base_channels * 4, depth=1, kernel_size=3, patch_size=[1, 3, 5], reduction=16)
        # self.multi_seam_p4 = MultiSEAM(c1=base_channels * 8, depth=1, kernel_size=3, patch_size=[1, 3, 5], reduction=16)
        # self.multi_seam_p5 = MultiSEAM(c1=base_channels * 16, depth=1, kernel_size=3, patch_size=[3, 5, 7], reduction=16)
    ...
    def forward(self, x):
        ...
        P5          = self.conv_for_feat3(feat3)  # 256
        # 20, 20, 512 -> 40, 40, 512
        ...
        """应用 MultiSEAM 注意力机制"""
        # P3 = self.multi_seam_p3(P3)
        # P4 = self.multi_seam_p4(P4)
        # P5 = self.multi_seam_p5(P5)
